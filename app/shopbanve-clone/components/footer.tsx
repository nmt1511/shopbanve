    ...
    <footer className="bg-[#172554] text-white">
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_0.7fr_0.7fr_1fr]">
          <div><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#f97316] text-sm font-black">SB</span><span className="text-lg font-black">{settings.shopName.toUpperCase()}</span></div><p className="mt-5 max-w-sm text-sm leading-7 text-blue-200">Nền tảng tài liệu giúp bạn tìm kiếm, học tập và làm việc với dự án hiệu quả hơn.</p><div className="mt-6 flex gap-3"><a href="https://www.facebook.com/" target="_blank" rel="noreferrer" aria-label="Facebook" className="rounded-lg bg-white/10 p-2.5 transition hover:bg-[#f97316]"><Facebook className="h-4 w-4" /></a><a href="https://www.instagram.com/" target="_blank" rel="noreferrer" aria-label="Instagram" className="rounded-lg bg-white/10 p-2.5 transition hover:bg-[#f97316]"><Instagram className="h-4 w-4" /></a></div></div>
          ...
          <div><h3 className="font-bold">Nhận cập nhật mới</h3><p className="mt-3 text-sm leading-6 text-blue-200">Đăng ký để không bỏ lỡ tài nguyên và bài viết hữu ích.</p><Link href="/lien-he" className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#f97316] px-4 py-3 text-sm font-bold text-white transition hover:bg-orange-500">Liên hệ để nhận cập nhật <ArrowRight className="h-4 w-4" /></Link></div>
        </div>
